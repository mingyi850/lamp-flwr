...
def reconstruct(args, device, sample, metric, tokenizer, lm, model_main, model_local):
    ...
    bert_embeddings = model_local.get_input_embeddings()
    ...
    true_grads = compute_grads(model_local, true_embeds, true_labels)
    ...
    x_embeds = get_init(args, model_main, unused_tokens, true_embeds.shape, true_labels, true_grads, bert_embeddings,
    ...
    for it in range(args.n_steps):
        ...
        def closure():
            opt.zero_grad()
            rec_loss = get_reconstruction_loss(model_main, x_embeds, true_labels, true_grads, args, create_graph=True)
        ...
        if args.use_swaps and it >= args.swap_burnin * args.n_steps and it % args.swap_every == 1:
            swap_tokens(args, x_embeds, max_len, cos_ids, lm, model_main, true_labels, true_grads)
        ...
        if steps_done % args.print_every == 0:
            ...
            _, _, tot_loss_proj = get_loss(args, lm, model_main, cos_ids, x_embeds_proj, true_labels, true_grads)
            perplexity, rec_loss, tot_loss = get_loss(args, lm, model_main, cos_ids, x_embeds, true_labels, true_grads)
    ...
    _, _, best_tot_loss = get_loss(args, lm, model_main, cos_ids, x_embeds_proj, true_labels, true_grads)
    best_ids = cos_ids
...
def check_path_and_raise_error(path):
    if not os.path.isdir(path):
        raise FileNotFoundError(f"No corresponding model found at {path}")
...
def main():
    ...
    path_main = f'SYNC/{args.dataset}/noise_{args.noise}/{args.steps+args.stillness}'
    path_local = f'SYNC/{args.dataset}/noise_{args.noise}/{args.steps}'

    # Checking the paths
    try:
        check_path_and_raise_error(path_main)
        check_path_and_raise_error(path_local)
        print("Both paths exist and contain folders.")
    except FileNotFoundError as e:
        print(e)  # Print the error message

    model_main = AutoModelForSequenceClassification.from_pretrained(path_main).to(device)
    model_main.eval()

    model_local = AutoModelForSequenceClassification.from_pretrained(path_local).to(device)
    model_local.eval()
    ...
    for i in range(args.n_inputs):
        ...
        prediction, reference = reconstruct(args, device, sample, metric, tokenizer, lm, model_main, model_local)
