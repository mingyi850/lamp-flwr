...
def reconstruct(args, device, sample, metric, tokenizer, lm, model):
    ...
    #decide on optimizer to use for reconstruction
    bert_embeddings_weight = bert_embeddings.weight.unsqueeze(0)
    ...
    for it in range(args.n_steps):
        ...
        # Tentative - calculate loss using initialized x_embeds,  continuous optimization
        def closure():
            opt.zero_grad()
            # Reconstruction loss
            rec_loss = get_reconstruction_loss(model, x_embeds, true_labels, true_grads, args, create_graph=True)
            # Regularization loss (prevents large embeddings (sentences))
        ...
        # Optimizer step calculates the loss (using the closure) and updates the tensor (which is the parameters to be optimized)
        ...
        # Finds the closest tokens to x_embeds and updates it in cos_ids
        _, cos_ids = get_closest_tokens(x_embeds, unused_tokens, bert_embeddings_weight)
        
        # Trying swaps of token position and prefixes - also updates x_embeds to best_x_embeds
...
def main():
    ...
    print("Running on device from args", args.device)
    metric = load_metric('rouge', experiment_id=(args.dataset + str(args.batch_size)))
